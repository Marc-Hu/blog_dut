...
RETURNS TABLE (id integer,id_auth integer, name varchar, created_at timestamp, updated_at timestamp, tag integer) AS
$$
  SELECT suj_id, suj_author, suj_name, suj_created_at, suj_updated_at, suj_tag 
    FROM get_sujets
      WHERE suj_author=$1;
$$ language SQL;
